...
func (uq *UserQuery) QueryNodes() *NodeQuery {
	query := (&NodeClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithNodes(opts ...func(*NodeQuery)) *UserQuery {
	query := (&NodeClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withNodes; query != nil {
		if err := uq.loadNodes(ctx, query, nodes,
			func(n *User) { n.Edges.Nodes = []*Node{} },
			func(n *User, e *Node) { n.Edges.Nodes = append(n.Edges.Nodes, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadNodes(ctx context.Context, query *NodeQuery, nodes []*User, init func(*User), assign func(*User, *Node)) error {
	...
	query.Where(predicate.Node(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.NodesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_nodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_nodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_nodes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
