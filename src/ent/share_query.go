...
func (sq *ShareQuery) QueryOwner() *UserQuery {
...
func (sq *ShareQuery) QueryNode() *NodeQuery {
...
func (sq *ShareQuery) WithOwner(opts ...func(*UserQuery)) *ShareQuery {
	query := (&UserClient{config: sq.config}).Query()
...
func (sq *ShareQuery) WithNode(opts ...func(*NodeQuery)) *ShareQuery {
	query := (&NodeClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ShareQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Share, error) {
	...
	if sq.withOwner != nil || sq.withNode != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withOwner; query != nil {
		if err := sq.loadOwner(ctx, query, nodes, nil,
			func(n *Share, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withNode; query != nil {
		if err := sq.loadNode(ctx, query, nodes, nil,
			func(n *Share, e *Node) { n.Edges.Node = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ShareQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Share, init func(*Share), assign func(*Share, *User)) error {
...
func (sq *ShareQuery) loadNode(ctx context.Context, query *NodeQuery, nodes []*Share, init func(*Share), assign func(*Share, *Node)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Share)
	for i := range nodes {
		if nodes[i].node_shares == nil {
			continue
		}
		fk := *nodes[i].node_shares
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "node_shares" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
