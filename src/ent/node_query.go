...
func (nq *NodeQuery) QueryOwner() *UserQuery {
...
func (nq *NodeQuery) QueryShares() *ShareQuery {
	query := (&ShareClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NodeQuery) WithOwner(opts ...func(*UserQuery)) *NodeQuery {
	query := (&UserClient{config: nq.config}).Query()
...
func (nq *NodeQuery) WithShares(opts ...func(*ShareQuery)) *NodeQuery {
	query := (&ShareClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if nq.withOwner != nil || nq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withOwner; query != nil {
		if err := nq.loadOwner(ctx, query, nodes, nil,
			func(n *Node, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := nq.withShares; query != nil {
		if err := nq.loadShares(ctx, query, nodes,
			func(n *Node) { n.Edges.Shares = []*Share{} },
			func(n *Node, e *Share) { n.Edges.Shares = append(n.Edges.Shares, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Node, init func(*Node), assign func(*Node, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].user_nodes == nil {
			continue
		}
		fk := *nodes[i].user_nodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NodeQuery) loadParent(ctx context.Context, query *NodeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Node)) error {
	...
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].node_parent == nil {
			continue
		}
		fk := *nodes[i].node_parent
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "node_parent" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NodeQuery) loadChildren(ctx context.Context, query *NodeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Node)) error {
	...
	for _, n := range neighbors {
		fk := n.node_parent
		if fk == nil {
			return fmt.Errorf(`foreign-key "node_parent" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "node_parent" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (nq *NodeQuery) loadShares(ctx context.Context, query *ShareQuery, nodes []*Node, init func(*Node), assign func(*Node, *Share)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Node)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Share(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.SharesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.node_shares
		if fk == nil {
			return fmt.Errorf(`foreign-key "node_shares" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "node_shares" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
